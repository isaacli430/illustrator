...
uniform vec4 ustartend;										// 0..1 range x=start, y=end, z=end-start
...
void main(void) {				
	...
	// mirror			
	vec2 uv2 = vec2( fract(v_uv_highlow.x) * 2.0, v_uv.w ); // v_uv.x premod
	uv2.x = 1.0 - abs(uv2.x-1.0);							// if ( uv2.x>1.0 ) uv2.x = 2.0 - uv2.x;	
	uv2.x = uv2.x*ustartend.z + ustartend.x;
	c = texture2D(t_tex, uv2);					
